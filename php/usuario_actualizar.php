<?php
    ...
    $check_admin=$check_admin->query("SELECT user,password FROM users WHERE user='$admin_usuario' AND id='".$_SESSION['id']."'");
    ...
    /*== Verificando usuario ==*/
    if($usuario!=$datos['user']){
	    $check_usuario=conexion();
	    $check_usuario=$check_usuario->query("SELECT user FROM users WHERE user='$usuario'");
	    if($check_usuario->rowCount()>0){
	        echo '
	            <div class="notification is-danger is-light">
	                <strong>¡Ocurrio un error inesperado!</strong><br>
    ...
    $actualizar_usuario=$actualizar_usuario->prepare("UPDATE users SET name=:nombre,last_name=:apellido,user=:usuario,password=:clave WHERE id=:id");
